...
ADD https://dlcdn.apache.org/tomcat/tomcat-9/v9.0.62/bin/apache-tomcat-9.0.62.tar.gz /tmp/tomcat9.tar.gz

RUN rm /usr/local/tomcat -rf \
    && tar zxvf /tmp/tomcat9.tar.gz -C /usr/local/ \
    && mv /usr/local/apache-tomcat-9.0.62 /usr/local/tomcat
...
RUN apk update; apk add ttf-dejavu ttf-freefont ttf-droid-nonlatin ttf-droid ttf-linux-libertine bash curl msttcorefonts-installer
